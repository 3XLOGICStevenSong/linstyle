						...
						var daterow = new Date(row.orderDate);
							...
							if(dateth.getTime() ==daterow.getTime()){
								if(row.serviceCategoryColor!=undefined){
									return "<a style='display: block;padding: 4px;' class='clicktd'>"+row.orderBeginTime+"-"+row.orderEndTime+"<br>"+row.age+"岁"+row.sex+"<br>"+row.orderNo+"</a>";
		...
		queryParams: function queryParams(params) {// 请求服务器数据时发送的参数，可以在这里添加额外的查询参数，返回false则终止请求
			...
			window.operateEvents = {
					...
					'click .clicktd': function (e, value, row, index) {
						window.location.href = "service-person-choose.html?id=" + row.id+"&selectFlag=1";
