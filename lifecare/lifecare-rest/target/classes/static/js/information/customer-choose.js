		...
		$("#table").bootstrapTable({ // 对应table标签的id
			...
			columns : [
				...
				},
				{
					...
					valign: 'middle',// 上下居中
					formatter: function (value, row, index) {
						return "<button type='button' class='btn btn-primary select data='" + row.userId + "'>选择</button></td>";
					},
...
window.operateEvents = {
		...
		'click .select': function (e, value, row, index) {
			...
			//console.info(window.btoa(window.encodeURIComponent(row.userId+"&servicePersonUserNumber="+row.userNumber+"&servicePersonRealName="+row.realName+"&servicePersonMobile="+row.mobile+"&servicePersonSex="+row.sex+"&servicePersonUserLevel="+row.userLevel+"&servicePersonAge="+row.age)));
			var passUndistributed = window.btoa(unescape(encodeURIComponent(paraUndistributed)));
			//"未发布任务添加画面"参数汉字转码
			var paraUnpublished = "userId=" + row.userId + "&userNumber=" + row.userNumber +"&realName=" + row.realName +"&sex="+row.sex+"&age="+row.age+"&userLevel="+row.userLevel+"&selectFlag=" + selectFlag;
			var passUnpublished = window.btoa(unescape(encodeURIComponent(paraUnpublished)));
			//返回"未分配任务添加画面"
			if (selectFlag == "1"){
