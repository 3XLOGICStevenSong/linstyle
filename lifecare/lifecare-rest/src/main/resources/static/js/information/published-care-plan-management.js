...
//            	searchBtn=1;
            ...
            //若有取消发布权限，unpublishBtn为1
            if(data[i]=="planend:unpublish"){
            	unpublishBtn=1;
            }
            //删除权限
	...
	function load(){
		...
		//获取session查询条件
		if(sessionStorage.getItem('pub_Plan_Cnds')!=null){
			var Srn_Cnds_JsonStr = sessionStorage.getItem('pub_Plan_Cnds');
			var pub_Plan_Cnds = JSON.parse(Srn_Cnds_JsonStr);
		    // 客户编号
			$("#userNumber").val(pub_Plan_Cnds.userNumber),
		    // 客户身份证号
			$("#idCard").val(pub_Plan_Cnds.idCard),
		    // 客户姓名
			$("#realName").val(pub_Plan_Cnds.realName),
		    // 客户手机号
			$("#mobile").val(pub_Plan_Cnds.mobile),   
		    // 客户生日
			$("#birthday").val(pub_Plan_Cnds.birthday), 
		    // 星级
			$("#userLevel").val(pub_Plan_Cnds.userLevel)
		}
		//获取session保存当前页码
		if(sessionStorage.getItem('publish_pn')==null){
			pn = 1
		}else{
			pn = parseInt(sessionStorage.getItem('publish_pn'))
		}
		...
		$("#table").bootstrapTable({// 对应table标签的id
			...
			columns: [
				 {
					...
					valign: 'middle',
					formatter: function (value, row, index) {
						return "<button tabIndex='8' type='button' class='btn btn-primary unpublished'  data='"+row.nursePlanId+"' >取消发布</button><button tabIndex='8' type='button' class='btn btn-primary btn-size btn-pos edit'  data='"+row.nursePlanId+"' >编辑</button><button type='button' class='btn btn-default btn-size btn-pos  delete' >删除</button>";
					},
			...
			onPageChange:function(number,size){
				sessionStorage.setItem('publish_pn',number,size);
			},
			//liuli 2019/02/01 end
			onLoadSuccess: function(data){
				...
				if(updateBtn==0&&unpublishBtn==0&&deleteBtn==0){
					//没有权限编辑也没有权限取消发布
	...
	window.operateEvents = {
		...
		'click .edit': function (e, value, row, index) {
			window.location.href = "published-care-plan-edit.html?"+window.btoa(unescape(encodeURIComponent("nursePlanId="+row.nursePlanId)));
		},
		//发布事件
...
//	});
	...
	$(".searchCarePlan").click(function(){
	    ...
	    if (formValidate().form()) { //判断校验是否符合规则
	    	...
	    	//以JSON格式保存
	    	sessionStorage.setItem('pub_Plan_Cnds', JSON.stringify(published_Plan_Conditions));
