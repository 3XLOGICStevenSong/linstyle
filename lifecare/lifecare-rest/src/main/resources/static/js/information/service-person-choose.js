...
//			 },
			 ...
			 },
			 {
					...
					valign: 'middle',
					formatter: function (value, row, index) {
						return "<button type='button' class=' btn btn-primary btn-default choice'>选择</button></td>"; 
				},
...
window.operateEvents = {
		//选择按钮点击事件
			'click .choice': function (e, value, row, index) {
				//selectFlag : 2(服务任务编辑选择)				
				if (selectFlag==2){
					var param="servicePersonId="+row.userId+"&servicePersonUserNumber="+row.userNumber+"&servicePersonRealName="+row.realName+"&servicePersonMobile="+row.mobile+"&servicePersonSex="+row.sex+"&servicePersonUserLevel="+row.userLevel+"&servicePersonAge="+row.age;
					//console.info(window.btoa(window.encodeURIComponent(param)));
				window.location.href = "service-task-edit.html?"+window.btoa(unescape(encodeURIComponent(param)));
				}
				//selectFlag : 1(服务任务池选择)
				else if(selectFlag==1){
					//用户主键
							...
							if(result.statusCode=="200"){
								//将token存入缓存	
			    				   ...
			    				   Swal.fire({
										...
										if (result.value) {
					    				    //跳转到服务人员管理页面
											window.location.href = "service-task-pool.html?"+window.btoa(unescape(encodeURIComponent("taskflag=1")));		
										}
									})
