...
public class UserLoginAction extends BaseAction {
	...
	public ActionForward doRegisterDoctor(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (!existUser) {
				...
				// 注册用户并且添加医生信息和医生定价
				paramEntity.setPassword(MD5Util.MD5(paramDTO.getPassword()));
				if(ResourceLocator.getI18NMessage(this, Constants.KEY_GENE).equals(paramDTO.getDcName())){
					//基因专家
					paramEntity.setDoctorType("1");
				}else{
					//普通医生
					paramEntity.setDoctorType("0");
				}
				iUserLoginService.addUserLogin(paramEntity);
			} else {
		...
		try {
			...
			if (resultEntity == null) { // 用户名不正确
				...
				if (!"".equals(resultEntity.getPassword())
					...
					//账号
					userLoginClientDTO.setUserCode(resultEntity.getUserTel());
					// 如果是医生获取医生信息
					...
					if ("0".equals(resultEntity.getRole())) {
						...
						doctorResult = iDoctorService.getDoctorLoginInfo(doctorEntity);
						if (doctorResult != null) {
							...
							userLoginClientDTO.setDoctorType(doctorResult.getDoctorType());
						}
	 ...
	 */
	// 登录
	public ActionForward doUpdatePassword(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if (paramDTO.getNewPassword()== null) {
			userLoginClientDTO.setReturnCode("-3094");
			toJson(response, userLoginClientDTO);
			return null;
		}
		// 参数Entity
		...
		try {
			...
			if (resultEntity == null) { // 用户名不正确
				...
				if(resultEntity.getPassword().equals(MD5Util.MD5(paramDTO.getPassword()))){
					paramEntity.setPassword(MD5Util.MD5(paramDTO.getNewPassword()));
					iUserLoginService.updateUserLogin(paramEntity);
				}else{
					userLoginClientDTO.setReturnCode("-3101");
					toJson(response, userLoginClientDTO);
					return null;
				}
			}
		} catch (Exception e) {
			userLoginClientDTO.setReturnCode("-3100");
			paramDTO.setErrFlg(true);
		}
