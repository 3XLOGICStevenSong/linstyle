$(function(){
	//var doctorId=getCookie("doctorId",doctorId);
	//http://192.168.11.102:8080/MedicalUnicom/HealthForWeb_App/Pat_Tiket.html?doctorId=1&time=2016-11-24%17:00&depId=1&depName=%E5%BF%83%E8%A1%80%E7%AE%A1%E5%86%85%E7%A7%91%E4%B8%93%E4%B8%9A
	//var depId=window.location.href.substring(window.location.href.indexOf("=")+1,window.location.href.lastIndexOf("&"));
	//var depName=decodeURI(window.location.href.substring(window.location.href.lastIndexOf("=")+1,window.location.href.length));
	...
	$.ajax({
		...
		success:function(result){
			...
			$(".SubmitBtn").click(function(){
				...
				var DocID=getCookie("DocID");
				var packageId=getCookie("packageId");
				var appointType=getCookie("appointType");
				var symptonId=getCookie("symptonID");
				var Tel=getCookie("Tel");
				var pack=getCookie("pack");

				$.ajax({
					type:"post",
					url:"/MedicalUnicom/Appoint.do?doInsertAppoint",
					data:{
						doctorId:DocID,
						patientId:pid,
						memo:memo,
					...
					success:function(result){
						...
						if(result.returnCode == "0"){
							//window.location.href="payment.html?id="+pack+"&doctorId="+DocID;
							window.location.href="payment.html?id="+pack+"&orderId="+orderId+"&ToFee="+fee+"&signedPayString="+signedPayString+"&doctorId="+doctorId;
						}
					}
				})
			})
