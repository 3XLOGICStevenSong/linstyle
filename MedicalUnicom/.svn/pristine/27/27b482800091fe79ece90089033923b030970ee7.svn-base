...
public class CheckUtil {
    ...
    public static boolean checkNumberByte(String strInput, int intMaxLength) {
        ...
        if (!checkStringSingle(strInput) || !checkNumber(strInput) || !checkLength(strInput, intMaxLength)) {
            return false;
        }
    ...
    public static boolean checkDateConsistency(String strInput) {
        ...
        if (strInput.length() == 10) {
            if (strInput.substring(4, 5).equals("/") && strInput.substring(7, 8).equals("/")) {
                ...
                if (intYear == cal.get(Calendar.YEAR) && (intMonth - 1) == cal.get(Calendar.MONTH) && intDay == cal.get(Calendar.DATE)) {
                    blnFlag = true;
                }
            }
        }
     ...
     *            日付(050801 / 0508)
     * 
     * @return blnFlg ture:OK　false:エラー Example: compareDate('050801','050901')
     */
